...
def run_pipeline(data_source: DataSource, position: PositionInfo | None = None) -> LiveDecisionResponse:
    ...
    data_age_sec = max(0.0, (now - datetime.fromisoformat(last_tick.replace("Z", "+00:00"))).total_seconds())
...
def _no_trade_response(reason: str, components: list[ComponentHealth], now: datetime) -> LiveDecisionResponse:
    ...
    return LiveDecisionResponse(
        timestamp=utcnow(), market={"status": "DATA_ERROR", "reason": reason}, regime={}, levels={},
